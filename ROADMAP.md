...
- [x] **M4 — NDJSON, plus JSON arrays**: `src/ndjson.zig`, wired into `Query` via a `Source` tagged union so `scan("data.csv")` and `scan("data.ndjson")` (format inferred from extension, or explicit override) behave identically at every layer — filter, projection, limit, and the newline-only `count()` fast path all work the same for both, since both formats are one-record-per-line. `.json` files are also routed to the NDJSON path; `src/json_array.zig` (also vendored from zson) sniffs the first non-whitespace byte and, for an array (`[{...},{...}]`), converts it to NDJSON text once up front — everything downstream (header derivation, filtering, `next()`) never knows the source was ever an array. Not streaming (the whole array is read and rewritten before scanning starts), but consistent with how this project already fully mmaps its source file regardless of format. Nested objects/arrays *as field values* are still out of scope and fail loudly (`NestedValueNotSupported`) rather than silently stringifying.
